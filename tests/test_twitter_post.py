...
def mock_env_vars(monkeypatch):
    monkeypatch.setenv('TWITTER_SECRET_ARN', 'arn:aws:secretsmanager:us-east-1:123:secret:test')
...
def s3_event():
    return {
        "Records": [{
            ...
            "s3": {
                "object": {"key": "images/january_15_0_NationalHatDay.jpg"}
            }
        }]
    }
...
def test_insert_space_before_capital(mock_env_vars, mock_twitter_creds):
    with patch('boto3.client') as mock_boto:
        mock_secrets = Mock()
        mock_secrets.get_secret_value.return_value = {'SecretString': json.dumps(mock_twitter_creds)}
...
def test_handler_success(mock_env_vars, mock_twitter_creds, s3_event, lambda_context):
    with patch('boto3.client') as mock_boto:
        ...
        def client_factory(service):
            ...
            if service == 'dynamodb': return mock_dynamodb
            return mock_secrets
...
def test_handler_s3_failure(mock_env_vars, mock_twitter_creds, s3_event, lambda_context):
    with patch('boto3.client') as mock_boto:
        mock_s3 = Mock()
        ...
        with patch('tweepy.Client'), patch('tweepy.OAuth1UserHandler'), \
             patch('tweepy.API'), patch('os.chdir'):
...
def test_update_dynamodb_record(mock_env_vars, mock_twitter_creds):
    with patch('boto3.client') as mock_boto:
        ...
        mock_boto.side_effect = lambda s: mock_dynamodb if s == 'dynamodb' else mock_secrets
