...
def mock_env_vars(monkeypatch):
    ...
    monkeypatch.setenv(
        'OPENAI_SECRET_ARN', 'arn:aws:secretsmanager:us-east-1:123:secret:test'
    )
    monkeypatch.setenv(
        'TWITTER_SECRET_ARN', 'arn:aws:secretsmanager:us-east-1:123:secret:twitter'
    )
...
def mock_config():
    ...
    day = str(now.day)
    return {'Prompt': 'Create a spooky image for ', month: {day: ['Hat', 'Bagel']}}
...
def test_handler_success(
    mock_env_vars, mock_config, mock_openai_response, lambda_context
):
    with patch('boto3.client') as mock_boto:
        mock_s3 = Mock()
        mock_s3.get_object.return_value = {
            'Body': Mock(read=lambda: json.dumps(mock_config).encode())
        }
        mock_dynamodb = Mock()
        mock_secrets = Mock()
